...
# Download the punkt tokenizer for sentence splitting in gTTS if not already present
...
# Load the main multimodal model and its processor
...
# Load the Whisper model and processor for speech-to-text
...
def transcribe(audio_path):
    """
    Transcribes audio from a given file path to text using the Whisper model.
    """
    if not audio_path:
        return ''
    try:
        # Load the audio file and resample to 16kHz as required by Whisper
...
def text_to_speech(text, file_path, language):
    """
    Converts a given text string to speech and saves it as an MP3 file.
    """
    lang_code_map = {
...
# Define reusable Gradio button states
...
def convert_history_to_messages(history):
    """
    Converts the application's state history into the message format
    expected by the Qwen-VL model processor.
    """
    messages = []
...
def convert_history_to_gradio(history):
    """
    Formats the state history for display in the Gradio Chatbot UI,
    including markdown for images and videos with the correct URL prefix.
    """
    gradio_chat = []
    for i in range(0, len(history), 2):
        user_turn = history[i]
        ...
        if user_turn.get('image_path'):
            ...
            # Add the required /gradio_api/ prefix for file URLs
...
def add_text(history, text, image, video, request: gr.Request):
    """
    Adds the user's new message (text, image, or video) to the chat history.
    Saves uploaded files to a temporary directory.
    """
    if not text and not image and not video:
        return history, convert_history_to_gradio(history), "", None, None, no_change_btn
    ...
    if video is not None:
        # Copy the uploaded video to a persistent temporary directory
        temp_video_name = os.path.basename(video)
        video_path = os.path.join(temp_dir, temp_video_name)
        shutil.copy(video, video_path)

    # Append the new user message and a placeholder for the assistant's reply
    ...
    # Clear input components and disable buttons while processing
    return history, chatbot_display, "", None, None, disable_btn

def qwen_bot(history, temperature, top_p, max_new_tokens, language, request: gr.Request):
    """
    Main function to generate a response from the Qwen-VL model.
    It processes the history, calls the model, and streams the output.
    """
    ...
    # Use the helper library to process images and videos from the message history
    image_inputs, video_inputs = process_vision_info(messages)
    
    # Conditionally build the arguments for the processor
    processor_kwargs = {
        "text": [text],
        "padding": True,
        "return_tensors": "pt"
    ...
    # Run model generation in a separate thread to keep the UI responsive
    ...
    for new_text in streamer:
        ...
        chatbot_display = convert_history_to_gradio(history)
        yield history, chatbot_display, disable_btn
        
    thread.join()
    final_response = history[-1]['content']
    chatbot_display = convert_history_to_gradio(history)
    yield history, chatbot_display, enable_btn
    
    # Generate text-to-speech for the final response if it's not empty
    if final_response:
        text_to_speech(final_response, "/home/cdsw/voicetmp.mp3", language)

def clear_history(request: gr.Request):
    """
    Clears the chat history and resets the UI components.
    """
    return [], [], "", None, None, enable_btn
...
def build_demo():
    """
    Builds the Gradio user interface for the application.
    """
    ...
    with gr.Blocks(title="Qwen-VL", theme=gr.themes.Default(), css=block_css) as demo:
        ...
        gr.Image(value="/home/cdsw/clouderalogo.png", height=35, show_label=False, show_download_button=False, interactive=False, container=False)
        
        with gr.Row():
            # Left column for inputs and parameters
            ...
            # Right column for the chatbot interface
            with gr.Column(scale=8):
                ...
                with gr.Row(elem_id="buttons"):
                    # The regenerate button has been removed from here
                    clear_btn = gr.Button(value="🗑️  Clear", interactive=False)

        # The regenerate button has been removed from the button list
        btn_list = [clear_btn]
        ...
        # Define the event chain for submitting a message
